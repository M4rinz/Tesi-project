...
In this first chapter the classic topic of floating point arithmetic is presented. It will start with a refresher of the well known basic principles, with the aim of presenting how arbitrary precision computation works in Julia.  

All the definitions in this chapter are formulated in terms of real quantities. Extending them to the complex case is generally straightforward, and the corresponding notions will be used throughout this essay without further comment whenever required.
The floating point number system presented in Section~\ref{sect:floatingpoint} is likewise introduced as a subset of $\R$, as it is customary. The reader is referred to~\cite[§~3.6]{higham:accuracy_book} for a discussion of complex floating point arithmetic.
...
Denormal numbers can cause troubles if one is unaware. They are outside the scope of this essay, thus they will not be treated any further, the focus will be on normalized number systems.
\smallskip 
...
\begin{oss*}
	...
	This intuitive explanation can be made more rigorous with the notion of \emph{ulp} (unit in last place). For $x\in\mathcal{F}$ as in~\eqref{eqn:float_pt_number}, $\mathtt{ulp}(x)=\beta^e\cdot m_0$ where $m_0$ is the significand whose digits are all zero except for the least significant one, which equals 1, i.e. $\mathtt{ulp}(x)=\beta^{e-t}$.
\end{oss*}
...
What has been discussed so far shows that the finiteness of $t$ is a limitation on precision, the finiteness of $e$ is a limitation on range.
%\footnote{As a curiosity, Google's \texttt{bfloat16} trades precision for range. More information can be found in \href{https://cloud.google.com/blog/products/ai-machine-learning/bfloat16-the-secret-to-high-performance-on-cloud-tpus}{the blog post that introduced the format}.} 
...
A quantity that is closely related to the machine precision is the \emph{unit roundoff}. The two terms are not defined in the \textsc{IEEE} 754 standard~\cite{article:IEEE754_2019}, and are often used interchangeably in the literature. There is no universal agreement on the definition of the two terms, for reasons that will be discussed shortly. This is rarely harmful, since the competing definitions differ by at most a factor of $2$.	
...
When equipped with the arithmetic operations inherited from $\R$, $\mathcal{F}$ lacks many useful mathematical properties. %In general, $\mathcal{F}(+,\cdot)$ has no algebraic structure. In particular, associativity is not guaranteed in $\mathcal{F}$ , and 
...
The standard model~\eqref{eqn:std_mdl_float_arithm} lies at the foundation of any error analysis. We note that $\mathcal{F}(+,\cdot)$ has no algebraic structure in general. In particular, operations between floating point numbers are not associative, and also the distributive property is lost.

The abstract models of floating point numbers and operations between them are behind concrete implementations. The \textsc{IEEE} standard 754~\cite{article:IEEE754_2019}, lastly revised in 2019, is the most popular technical standard for floating point arithmetic. The standard specifies: formats for binary and decimal floating point data, formats for data interchange and computation, conversion rules (e.g. between floating point formats), arithmetic operations and comparisons, rounding modes, floating point exceptions and their handling.  
...
\begin{oss}\label{oss:prec_vs_acc}
	Although the two terms are often confused and used interchangeably, precision and accuracy are not the same thing. Let us make a distinction between them by clarifying the terminology that will be used throughout this essay.
...
In \matlab{}, arbitrary precision arithmetic is commonly performed via the \texttt{vpa} command. A popular alternative is the commercial Multiprecision Computing Toolbox~\cite{matlab_mpct}. The toolbox introduces a floating-point numeric type, and provides mathematical functions that support it.
...
The \matlab{} implementation of the multiprecision algorithm developed by Fasi and Higham~\cite{higham:hf19_mpexpm}, which the authors themselves propose, relies on this toolbox.

\subsection{Multiple precision arithmetic in Julia}\label{subs:multiprecision_in_Julia}
...
\begin{lstlisting}[language=Julia]
  ...
  a = 74	
  let 
    a = 42	# the variable a is being reassigned
...
%\noindent Example~\ref{es:big_BigFloat_constructor} shows some subtleties in the usage of the \texttt{BigFloat()} and \texttt{big()} constructors.
...
\begin{esempio}\label{es:float_promotion}
	If $\mathtt{x}$ is not of type \texttt{BigFloat}, then the \texttt{big(x)} and \texttt{BigFloat(x)} constructor try to convert it. 
	The conversion of a double precision number is conceptually simple: the significand of $\mathtt{x}$ is preserved, and the extra lower-order significand bits in the high precision representation are set to zero.  
\begin{lstlisting}[language=Julia]
  ...
  big(0.0625)	# 1/16 is exactly representable 
  		# in binary
\end{lstlisting}
	This example shows that $\texttt{BigFloat}(0.1)$ and $\texttt{big}(0.1)$ create a high precision approximation (the default precision is 256 binary digits) of $\mathtt{fl}(0.1)$, not of $0.1\in\R$
\end{esempio}
\smallskip
...
\begin{oss*}
	...
	\begin{itemize}
		...
		\item Since \textsc{blas}~\cite{article:BLAS} subroutines for matrix multiplications are available only for single and double precision, the computation of $C=A\cdot B$ is implemented with the naive three-way for loop, using the $i,j,k$ ordering. Julia tries to exploit Instruction Level Parallelism by using the \texttt{@simd} macro on the innermost loop. $C_{i,j}$ is updated by using fused multiply-add operations, implemented by \textsc{mpfr}.
		\item $\mathtt{inv}(A)$ is implemented because the $\mathtt{lu}$ command works with $\mathtt{BigFloat}$ matrices out of the box. As before, no \textsc{lapack} routine can be used; the factorization is computed using the Gaussian elimination algorithm with pivoting. The lower level arithmetic operations are performed via \textsc{mpfr} calls. 
	...
	\end{itemize}
\end{oss*}
If the exponent $n$ is not an integer, the computation of the matrix power $A^n$ is not natively available. This is because $A^n$ is computed as $Q\,T^n\,T^r\,Q^{h}$, where $A=QTQ^H$ is a Schur decomposition of $A$, $T^r$ with $r=n-\lfloor n\rfloor$ is computed using the algorithm described in~\cite{higham:hili13}.
...
\begin{itemize}
	\item The \emph{intrinsic error} is due to data uncertainty, namely by replacing, in the input to the algorithm, a real quantity with a finite precision one. If present, this type of error would influence the output of an algorithm even if all the operations were performed exactly and there was no truncation.
...
\begin{definizione}\label{def:error}
	Let $\hat{y}\in\R{n}$ be a computed approximation to the quantity $y\in\R{n}$, let $\norm{\cdot}$ be a norm on $\R{n}$. The \emph{absolute} and \emph{relative} \emph{errors} in the computation of $y$ are given by
	\[
...
\begin{definizione}
	The \emph{backward error} in the computation of $y$ is the smallest perturbation $\Delta{x}$ such that $\hat{y}=f(x+\Delta{x})$. There may be many $\Delta{x}$ that satisfy the relation, or there may be none at all. We therefore define the backward error more precisely as
	\[
		E_{\textup{back}}(\hat{y})=\min\qty{\frac{\norm{\Delta{x}}}{\norm{x}}\mid\hat{y}=f(x+\Delta{x})}
	\]
\end{definizione}

When evaluating the quality of $\hat{y}$, we would like the relative error to be of the order of machine precision. For reasons that have to do with the sensitivity of the problem, as it will be explained shortly, this can't always be achieved. This doesn't mean that there are no good solutions at all. 
The \emph{backward error analysis} can be sometimes helpful.
...
In backward error analysis, the error is being interpreted as perturbations in the input. Since the data may be affected by tiny errors anyways, a solution $\hat{y}$ returned by a backward stable algorithm is often more than enough; $\hat{y}$ is ``as good as it can be''.

\begin{oss*}
	A backward error analysis can't always be performed. A famous example is the computation of the outer product $xy^{\top}$, for $x,y\in\R{n}$. In fact the computed $\widehat{xy^{\top}}$ satisfies
	\[
		\qty(\widehat{xy^{\top}})_{i,j}=x_iy_j(1+\delta_{i,j})\qquad\abs{\delta_{i,j}}\le u\quad\forall\,i,j\in[n],
	\]
	therefore it will not be a rank 1 matrix in general. Instead, if we consider the exact outer product of two perturbations $\tilde{x}$ and $\tilde{y}$ of $x$ and $y$ respectiverly, we have that $\tilde{x}\tilde{y}^{\top}=(x+\Delta{x})(y+\Delta{y})^{\top}$ will be of rank 1.
\end{oss*}

\begin{oss*}
	We remark that, in principle, the backward error analysis is a technique that can be applied to any source of error, albeit not always successfully. 
	
	It is often the roundoff error that is analyzed by means of the backward error, an example being the QR factorization of a square matrix. As we will see in Section~\vref{subs:scaling_and_squaring}, the scaling and squaring method to compute the matrix exponential, a central topic of this essay, gives a prominent example of backward error analysis on the truncation error. 
\end{oss*}

\section{Conditioning}\label{sect:conditioning_general}
...
\end{definizione}
Definition~\ref{def:cond_number} can be easily adapted to other contexts such as real functions of a real variable. In particular, it will be adapted to the context of matrix functions in Section~\ref{sect:cond_matfun}.
...
\begin{notazione}\label{notaz:kappa_f}
	Throughout the document, the relative condition number of $f$ at $x$ will be denoted as $\kappa_{f}(x)$. The used norm will be desumed by the context, even in the more delicate case in which $f$ is a function of a matrix (see Equation~\eqref{eqn:cond_frechet} and the discussion thereafter). When $A$ is a matrix, $\kappa(A)$ will denote its condition number.
\end{notazione}
...
\]
By using the \emph{Fréchet derivative}, these expressions will be generalized to matrix functions in Section~\ref{sect:cond_matfun} (see Equation~\eqref{eqn:cond_frechet})
...
From Equation~\eqref{eqn:backerr_fwerr_cond_relation}, we get the useful rule of thumb $E_{\text{rel}}\lesssim\kappa\cdot E_{\text{back}}$.
