...
Pₘ  = horner(A, pade_num_coeffs, s);
Qₘ  = horner(A, pade_den_coeffs, s);
Y_h = Qₘ \ Pₘ;

#_    = expm_diagonal!(S_tayl, m, 0);    # Just to check if the error is caught
Y_ps = eval_pade!(S_pade, m, s);
...
S = AandPowsStruct(A, [I(n), A^2], false);

Y_true = exp(Float64.(A/2^s));
